...
const config = {
  ...
  get awsS3Command() {
    return `aws s3 sync --acl public-read --include 'webpackBuild/' webpackBuild s3://${this.bucket}${this.directory}`; // you can edit this if you have different s3 sync setup
  }
};
...
function main() {
  ...
  for (const prop in config) {
    if (config[prop] === null) {
      throw new Error(
        `You forgot to set up the global variable, friend. ${prop} is: ${config[prop]}`
      );
    }
  }
  ...
  const deployCommand = `${config.buildCommand} && ${config.awsS3Command}${branchName}`;
