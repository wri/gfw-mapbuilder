...
class TabViewContainer extends React.Component<TabViewContainerProps, {}> {
  render() {
    const TabViews = this.props.tabViewsToRender.map((tabView: TabViewRender) => {
      switch (tabView.label) {
        ...
        case 'analysis':
          return <AnalysisTabView label={tabView.label} key={tabView.label} />;
        case 'documents':
          return <DocumentsTabView label={tabView.label} key={tabView.label} />;
        case 'menu':
