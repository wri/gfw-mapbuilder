...
const getTimeInfo = operationalLayer => {
  return (
    operationalLayer.resourceInfo && operationalLayer.resourceInfo.timeInfo
  );
};
...
export default class Map extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      (prevProps.activeWebmap !== undefined &&
        prevProps.activeWebmap !== activeWebmap)
    ) {
    ...
    if (
      (prevState.basemap !== basemap || prevState.map !== map) &&
      map.loaded
    ) {
      basemapUtils.updateBasemap(
        map,
        basemap,
        settings.layerPanel.GROUP_BASEMAP.layers
      );
    }
  ...
  createMap = (webmap, options) => {
    ...
    arcgisUtils
      .createMap(webmap, this.refs.map, {
        mapOptions: options,
      ...
      .then(response => {
        ...
        response.map.infoWindow.on(
          "show, hide, set-features, selection-change",
          mapActions.infoWindowUpdated
        );
        ...
        on.once(response.map, "update-end", () => {
          ...
          const cDensityFromHash = this.applyLayerStateFromUrl(
            response.map,
            itemData
          );
          ...
          if (settings.iso && settings.iso !== "") {
            ...
            if (maskLayer) {
              ...
              maskLayer.visibleLayers.forEach(layerNum => {
                layerDefs[layerNum] = `code_iso3 <> '${encodeURIComponent(
                  settings.iso
                )}'`;
              });
          ...
          response.map.on("extent-change", evt => {
            const imageryLayer = response.map.getLayer(
              layerKeys.RECENT_IMAGERY
            );
            ...
            if (wmsLayers.length) {
              wmsClick(evt, wmsLayers, brApp.map.extent).then(responses => {
                ...
                Object.keys(responses).forEach(layerId => {
                  if (
                    Array.isArray(responses[layerId]) &&
                    responses[layerId].length > 0
                  ) {
          ...
          const userFeaturesLayer = response.map.getLayer(
          ...
          editToolbar.on("deactivate", evt => {
            if (evt.info.isModified) {
              ...
              const matchingUserSubscriptions = userSubscriptions.filter(
                userSubscription => {
                  return (
                    userSubscription &&
                    evt.graphic.attributes &&
                    userSubscription.attributes.params.geostore ===
                      evt.graphic.attributes.geostoreId
                  );
                }
              );
              ...
              analysisUtils.registerGeom(evt.graphic.geometry).then(res => {
                ...
                if (matchingUserSubscriptions.length > 0) {
                  this.updateThenDeleteSubscription(
                    res.data.id,
                    matchingUserSubscriptions[0]
          ...
          layersHelper.updateAGBiomassLayer(
            cDensityFromHash ? cDensityFromHash : canopyDensity,
            response.map
          );
        });
        ...
        if (
          ...
          response.map.centerAndZoom(
            new Point(initialExtent.x, initialExtent.y),
            initialExtent.z
          );
        } else {
  ...
  updateThenDeleteSubscription = (registeredGeomId, userSubscription) => {
    ...
    this.deleteSubscription(userSubscription.id).then(() => {
      this.updateSubscription(jsonData).then(newId => {
        ...
        const index = this.state.userSubscriptions
          .map(e => e.id)
          .indexOf(userSubscription.id);
  ...
  applyLayerStateFromUrl = (map, itemData) => {
    ...
    if (params.a) {
      ...
      const webmapLayerIds = webmapLayerConfigs.map(config =>
        config.subId ? config.subId : config.id
      );
      ...
      layerIds.forEach((layerId, j) => {
        ...
        if (opacityValues[j] && opacityValues[j] !== 1) {
          ...
          const dynamicLayers = [
            layerKeys.MODIS_ACTIVE_FIRES,
            layerKeys.VIIRS_ACTIVE_FIRES,
            layerKeys.IMAZON_SAD
          ];

          if (
            (mapLayer &&
              !mapLayer.setLayerDrawingOptions &&
              mapLayer.setOpacity) ||
            (mapLayer && dynamicLayers.indexOf(mapLayer.id) > -1)
          ) {
            ...
            mapLayer.visibleLayers.forEach(visibleLayer => {
              options[visibleLayer] = new LayerDrawingOptions({
                transparency: 100 - opacityValues[j] * 100
              });
            });
      ...
      if (webmapLayerIds.length > 0) {
        ...
        webmapLayerConfigs.forEach(webmapLayerConfig => {
          if (webmapLayerConfig.subIndex === undefined) {
            ...
            if (
              webmapLayerConfig.visible &&
              layerIds.indexOf(webmapLayerConfig.id) === -1
            ) {
              featLayer.hide();
            ...
            if (
              (layerIds.indexOf(webmapLayerConfig.subId) === -1 &&
                webmapLayerConfig.visible) ||
              (layerIds.indexOf(webmapLayerConfig.subId) > -1 &&
                !webmapLayerConfig.visible)
            ) {
              ...
              if (
                ...
                webmapIdConfig[webmapLayerConfig.id].layersToHide.push(
                  webmapLayerConfig.subIndex
                );
              } else {
                webmapIdConfig[webmapLayerConfig.id].layersToShow.push(
                  webmapLayerConfig.subIndex
                );
              }
            }
        ...
        Object.keys(webmapIdConfig).forEach(webmapId => {
          ...
          webmapIdConfig[webmapId].layersToHide.forEach(layerToHide => {
            updateableVisibleLayers.splice(
              updateableVisibleLayers.indexOf(layerToHide),
              1
            );
            const subLayerConfig = utils.getObject(
              webmapLayerConfigs,
              "subId",
              `${webmapId}_${layerToHide}`
            );
    ...
    if (params.ts && params.te) {
      layerActions.updateTerraIStartDate.defer(
        new Date(params.ts.replace(/-/g, "/"))
      );
      layerActions.updateTerraIEndDate.defer(
        new Date(params.te.replace(/-/g, "/"))
      );
    }
    ...
    if (params.ism && params.iem && params.isy && params.iey) {
      mapActions.updateImazonAlertSettings(
        actionTypes.UPDATE_IMAZON_START_MONTH,
        parseInt(params.ism)
      );
      mapActions.updateImazonAlertSettings(
        actionTypes.UPDATE_IMAZON_END_MONTH,
        parseInt(params.iem)
      );
      mapActions.updateImazonAlertSettings(
        actionTypes.UPDATE_IMAZON_START_YEAR,
        parseInt(params.isy)
      );
      mapActions.updateImazonAlertSettings(
        actionTypes.UPDATE_IMAZON_END_YEAR,
        parseInt(params.iey)
      );
    }
  ...
  addLayersToLayerPanel = (settings, operationalLayers) => {
    ...
    const saveLayersInOtherLang =
      ...
      settings.alternativeLanguage && settings.useAlternativeLanguage;
    ...
    if (settings.includeCartoTemplateLayers) {
      ...
      const cartoGroup = setupCartoLayers(
        cartoUser,
        cartoTemplateId,
        cartoApiKey,
        cartoGroupLabel,
    ...
    operationalLayers.forEach((layer, layerIndex) => {
      if (
        ...
        layer.resourceInfo.layers
      ) {
        const dynamicLayers = [];
        layer.resourceInfo.layers.forEach((sublayer, sublayerIndex) => {
          const visible =
            layer.layerObject.visibleLayers.indexOf(sublayer.id) > -1;
          const scaleDependency =
            sublayer.minScale > 0 || sublayer.maxScale > 0;
          const layerInfo = {
            id: layer.id,
        ...
        layer.featureCollection.layers.forEach(sublayer => {
          ...
          sublayer.layerObject.setOpacity(sublayer.opacity);
          layers.unshift(layerInfo);
          if (layerInfo.visible) {
            layerActions.addActiveLayer(layerInfo.id);
          }
        });
      } else {
        const layerInfo = {
    ...
    const groupKeys = Object.keys(settings.layerPanel).filter(
      g => g !== layerKeys.EXTRA_LAYERS && g !== layerKeys.GROUP_BASEMAP
    );
        ...
        case "radio": {
          ...
          if (group.layers.length) {
            ...
            layersFromWebmap.forEach(l => {
              if (l.hasOwnProperty("includedSublayers")) {
                // this is a dynamic layer
                layers.forEach(webmapLayer => {
                  if (
                    l.id === webmapLayer.id &&
                    l.includedSublayers.indexOf(webmapLayer.subIndex) > -1
                  ) {
          ...
          groupLayers.forEach(gl => {
            const layerConfigToReplace = utils.getObject(
              group.layers,
              "id",
              gl.id
            );
            group.layers.splice(
              group.layers.indexOf(layerConfigToReplace),
              1,
              gl
            );
          });
          ...
          group.layers.forEach(l => {
            l.isRadioLayer = true;
            if (exclusiveLayerIds.indexOf(l.id) === -1) {
            ...
            .forEach(l => {
              if (l.includedSublayers && l.includedSublayers.length > 0) {
                l.includedSublayers.forEach(sublayer => {
                  const mapLayer = layers.filter(
                    l2 => l2.id === l.id && l2.subIndex === sublayer
                  )[0];
        ...
        case "nested":
          {
            group.layers.forEach(nestedGroup => {
              if (!nestedGroup.hasOwnProperty("nestedLayers")) {
                throw new Error(
                  `nested groups must contain a 'nestedLayers' property. You may have made a configuration error. Check the 'resources.js' file`
                );
              }
              const layersFromWebmap = nestedGroup.nestedLayers
                .filter(nl => !nl.url)
                .map(l => {
                  const mapLayer = layers.filter(l2 => l2.id === l.id)[0];
              ...
              layersFromWebmap.forEach(nl => {
                ...
                nestedGroup.nestedLayers = [
                  ...nestedGroup.nestedLayers.slice(
                    0,
                    nestedGroup.nestedLayers.indexOf(layerConfigToReplace)
                  ),
                  nl,
                  ...nestedGroup.nestedLayers.slice(
                    nestedGroup.nestedLayers.indexOf(layerConfigToReplace) + 1
                  )
                ];
              });
            });
  ...
  render() {
    ...
    const timeSlider =
      webmapInfo && webmapInfo.widgets && webmapInfo.widgets.timeSlider;
    const timeWidgets = [];
    ...
    return (
      <div className={`map-container ${!timeSlider ? "noSlider" : ""}`}>
        ...
        <div
          className={`analysis-modal-container modal-wrapper ${
            analysisModalVisible ? "" : "hidden"
          }`}
        >
          <AnalysisModal drawButtonActive={this.state.drawButtonActive} />
        </div>
        <div
          className={`print-modal-container modal-wrapper ${
            printModalVisible ? "" : "hidden"
          }`}
        >
          <PrintModal />
        </div>
        <div
          className={`search-modal-container modal-wrapper ${
            searchModalVisible ? "" : "hidden"
          }`}
        >
          <SearchModal />
        </div>
        <div
          className={`canopy-modal-container modal-wrapper ${
            canopyModalVisible ? "" : "hidden"
          }`}
        >
          <CanopyModal canopyDensity={this.state.canopyDensity} />
        </div>
        <div
          className={`layer-modal-container modal-wrapper ${
            layerModalVisible ? "" : "hidden"
          }`}
        >
          <LayerModal info={modalLayerInfo} />
        </div>
        <div
          className={`subscription-modal-container modal-wrapper ${
            subscriptionsModalVisible ? "" : "hidden"
          }`}
        >
        ...
        <div
          className={`imagery-modal-container ${
            imageryModalVisible ? "" : "collapse"
          }`}
        >
          <ImageryModal
